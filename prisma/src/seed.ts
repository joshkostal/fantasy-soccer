...
const main = async () => {
  ...
  const fantasyLeagueScoreSettings =
    await prisma.fantasyLeagueScoreSettings.create({
      data: {
  ...
  const positionScoreMultiplier =
    await prisma.positionScoreMultiplier.createMany({
      data: [
        {
          positionId: 1,
          goal: 1,
